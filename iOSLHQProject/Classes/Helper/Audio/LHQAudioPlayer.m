...
@implementation LHQAudioPlayer

+ (instancetype)sharedInstance {
    static LHQAudioPlayer *player = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        player = [LHQAudioPlayer new];
    });
    return player;
}
...
- (instancetype)init {
    if (self = [super init]) {
        self.playerState = LHQAudioPlayerStateStopped;
    }
    return self;
}
...
- (void)play {
    if (self.playerState == LHQAudioPlayerStatePlaying) return;
    ...
    // 如果缓冲未完成
    if (self.bufferState != LHQAudioBufferStateFinished) {
        self.bufferState = LHQAudioBufferStateNone;
...
- (void)pause {
    if (self.playerState == LHQAudioPlayerStatePaused) return;
    
    self.playerState = LHQAudioPlayerStatePaused;
...
- (void)stop {
    if (self.playerState == LHQAudioPlayerStateStoppedBy) return;
    
    self.playerState = LHQAudioPlayerStateStoppedBy;
    [self setupPlayerState:self.playerState];
...
- (void)startTimer {
    ...
    self.playTimer = [LHQTimer scheduledTimerWithTimeInterval:1.0f target:self selector:@selector(timerAction:) userInfo:nil repeats:YES];
}
...
- (void)startBufferTimer {
    ...
    self.bufferTimer = [LHQTimer scheduledTimerWithTimeInterval:0.1f target:self selector:@selector(bufferTimerAction:) userInfo:nil repeats:YES];
}
...
- (void)timerAction:(id)sender {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if ([self.delegate respondsToSelector:@selector(lhqPlayer:currentTime:totalTime:progress:)]) {
            [self.delegate lhqPlayer:self currentTime:currentTime totalTime:totalTime progress:progress];
        }
        
        if ([self.delegate respondsToSelector:@selector(lhqPlayer:totalTime:)]) {
            [self.delegate lhqPlayer:self totalTime:totalTime];
        }
    });
}
...
- (void)bufferTimerAction:(id)sender {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (bufferProgress > 0.9 && buffer >= 1) {
            self.bufferState = LHQAudioBufferStateFinished;
            ...
            self.bufferState = LHQAudioBufferStateBuffering;
        }
        
        if ([self.delegate respondsToSelector:@selector(lhqPlayer:bufferProgress:)]) {
            [self.delegate lhqPlayer:self bufferProgress:bufferProgress];
        }
    });
}
...
- (void)setupPlayerState:(LHQAudioPlayerState)state {
    if ([self.delegate respondsToSelector:@selector(lhqPlayer:statusChanged:)]) {
        [self.delegate lhqPlayer:self statusChanged:state];
    }
}
        ...
        _audioStream.onStateChange = ^(FSAudioStreamState state) {
            switch (state) {
                case kFsAudioStreamRetrievingURL:       // 检索url
                    NSLog(@"检索url");
                    weakSelf.playerState = LHQAudioPlayerStateLoading;
                    break;
                case kFsAudioStreamBuffering:           // 缓冲
                    NSLog(@"缓冲中。。");
                    weakSelf.playerState = LHQAudioPlayerStateBuffering;
                    weakSelf.bufferState = LHQAudioBufferStateBuffering;
                    break;
                ...
                case kFsAudioStreamPlaying:             // 播放
                    NSLog(@"播放中。。");
                    weakSelf.playerState = LHQAudioPlayerStatePlaying;
                    break;
                case kFsAudioStreamPaused:              // 暂停
                    NSLog(@"播放暂停");
                    weakSelf.playerState = LHQAudioPlayerStatePaused;
                    break;
                case kFsAudioStreamStopped:              // 停止
                    
                    // 切换歌曲时主动调用停止方法也会走这里，所以这里添加判断，区分是切换歌曲还是被打断等停止
                    if (weakSelf.playerState != LHQAudioPlayerStateStoppedBy && weakSelf.playerState != LHQAudioPlayerStateEnded) {
                        NSLog(@"播放停止被打断");
                        weakSelf.playerState = LHQAudioPlayerStateStopped;
                    }
                    break;
                case kFsAudioStreamRetryingFailed:              // 检索失败
                    NSLog(@"检索失败");
                    weakSelf.playerState = LHQAudioPlayerStateError;
                    break;
                ...
                case kFsAudioStreamPlaybackCompleted:           // 播放完成
                    NSLog(@"播放完成");
                    weakSelf.playerState = LHQAudioPlayerStateEnded;
                    break;
                ...
                case kFSAudioStreamEndOfFile:                   // 缓冲结束
                    {
                        ...
                        if (self.bufferState == LHQAudioBufferStateFinished) return;
