...
#import "LHQRecorder.h"
#import <AVFoundation/AVFoundation.h>
#import "LHQThread.h"
#import <lame/lame.h>

#define RDAudioRecorderCache @"com.zxl.chat.RDAudioRecorderCache"

@interface LHQRecorder()
...
@property (nonatomic, strong) LHQThread *thread; //控制线程中录音转MP3 暂停和继续
@property (nonatomic, copy) NSString    *fileName;

@end

@implementation LHQRecorder

#pragma mark - Init Methods
- (instancetype)initWithfileName:(NSString *)fileName Delegate:(id<LHQRecorderDelegate>)delegate{
    if (self = [super init]) {
        _delegate = delegate;
...
-(LHQThread *)thread{
    if (!_thread) {
        _thread = [LHQThread currentThread];
    }
    return _thread;
}
...
- (NSString *)mp3Path {
    NSString *mp3Path = [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) firstObject] stringByAppendingPathComponent:self.fileName];
...
//    NSString *fileName = [NSString stringWithFormat:@"%.0f.mp3",timeInterval];
    NSString *fileName = [NSString stringWithFormat:@"%@.mp3",_audioName];
    return fileName;
}
...
// 获取指定文件夹下的所有文件
- (NSArray *)getDirectoriesAllFileWithPath:(NSString *)path{
