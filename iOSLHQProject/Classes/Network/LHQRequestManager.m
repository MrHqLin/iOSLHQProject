...
- (void)request:(NSString *)method URL:(NSString *)urlString parameters:(id)parameters completion:(void (^)(LHQBaseResponse *response))completion
{
    if (self.isLocal) {
    ...
    if (self.reachabilityManager.networkReachabilityStatus == AFNetworkReachabilityStatusNotReachable) {
        LHQBaseResponse *response = [LHQBaseResponse new];
...
#pragma mark - 加载本地数据
static NSString *jsonFileDirectory = @"LHQLocalJsons";
- (void)requestLocal:(NSString *)urlString completion:(void (^)(LHQBaseResponse *response))completion
{
...
#pragma mark - 处理数据
- (void)wrapperTask:(NSURLSessionDataTask *)task responseObject:(id)responseObject error:(NSError *)error completion:(void (^)(LHQBaseResponse *response))completion
{
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        
        LHQBaseResponse *response = [self convertTask:task responseObject:responseObject error:error];
...
#pragma mark - 包装返回的数据
- (LHQBaseResponse *)convertTask:(NSURLSessionDataTask *)task responseObject:(id)responseObject error:(NSError *)error
{
...
- (void)upload:(NSString *)urlString parameters:(id)parameters formDataBlock:(NSDictionary<NSData *, LHQDataName *> *(^)(id<AFMultipartFormData> formData, NSMutableDictionary<NSData *, LHQDataName *> *needFillDataDict))formDataBlock progress:(void (^)(NSProgress *progress))progress completion:(void (^)(LHQBaseResponse *response))completion
{
    ...
    [self POST:urlString parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        if (datas) {
            [datas enumerateKeysAndObjectsUsingBlock:^(NSData * _Nonnull data, LHQDataName * _Nonnull name, BOOL * _Nonnull stop) {
...
- (void)configSettings
{
    ...
    //自定义处理数据
    self.responseFormat = ^LHQBaseResponse *(LHQBaseResponse *response) {
        return response;
    };
}
...
+ (instancetype)manager {
    LHQRequestManager *manager = [super manager];
    [manager configSettings];
    return manager;
}

static LHQRequestManager *_instance = nil;
