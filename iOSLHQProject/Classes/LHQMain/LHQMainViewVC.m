...
@interface LHQMainViewVC () <UIGestureRecognizerDelegate>
{
...
- (void) handlePan: (UIPanGestureRecognizer *)rec{
    ...
    if (((self.mainVC.view.lhq_x <= 0) && (_scalef <= 0)) || ((self.mainVC.view.lhq_x >= (kScreenWidth - kMainPageDistance )) && (_scalef >= 0)))
    {
        //边界值管控
        _scalef = 0;
        needMoveWithTap = NO;
...
//        self.contentView.alpha = tempAlpha;
    ...
    {
        //超出范围，
        if (self.mainVC.view.lhq_x < 0)
        {
            [self closeLeftView];
            _scalef = 0;
        }
        else if (self.mainVC.view.lhq_x > (kScreenWidth - kMainPageDistance))
        {
            [self openLeftView];
            _scalef = 0;
        }
