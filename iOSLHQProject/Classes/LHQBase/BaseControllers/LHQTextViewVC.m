...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    ...
    if (textField.isEmptyAutoEnable && (LHQIsEmpty(textField.text.stringByTrim) || LHQIsEmpty(current))) {
        if (LHQIsEmpty(current)) {
...
- (void)checkIsEmpty:(BOOL)isEmpty textField:(UITextField *)textField {
    if (LHQIsEmpty(self.requiredTextFields)) {
        return;
    }
    
    if ([self respondsToSelector:@selector(textViewControllerRelationButtons:)]) {
        if (LHQIsEmpty([self textViewControllerRelationButtons:self])) {
            return;
        }
    }else
    {
    ...
    if (!isEmpty) {
        [self.requiredTextFields enumerateObjectsUsingBlock:^(UITextField *obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if (obj != textField && LHQIsEmpty(obj.text.stringByTrim)) {
                isButtonEnabled = NO;
                *stop = YES;
            }
        }];
...
#pragma mark - LHQTextViewControllerTextField

static void *isEmptyAutoEnableKey = &isEmptyAutoEnableKey;
@implementation UITextField (LHQTextViewController)
...
@implementation LHQTextViewControllerTextField
- (NSString *)text {
