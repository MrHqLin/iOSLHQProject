...
#define kDefaultCancelBtnTitleFont [UIFont systemFontOfSize:15]

#define kDefaultCancelBtnTitleColor [UIColor colorWithRed:(102)/255.0 green:(102)/255.0 blue:(102)/255.0 alpha:1.0]

@interface LHQSearchBar () <UITextFieldDelegate>

@property (nonatomic, assign) BOOL hideClearBtn;

@end

@implementation LHQSearchBar


-(instancetype)initWithFrame:(CGRect)frame
{
    if (self == [super initWithFrame:frame]) {
//        [self setBackgroundColor:k_Color_979797];
       
        self.showsCancelButton = NO;
        //光标颜色tintColor
//         self.tintColor = k_Color_EFB02A;
        self.backgroundImage = [UIImage imageWithColor:[UIColor clearColor] size:self.bounds.size];
        self.backgroundColor = [UIColor clearColor];
        
        self.originPositionSearchOffSet = UIOffsetMake(AdaptedWidth(10), 0);
...
- (void)setCancelBtnTitleFont:(UIFont *)cancelBtnTitleFont{
    _cancelBtnTitleFont = cancelBtnTitleFont;
    UIButton *cancel = [self cancelBtn];
    if (cancel) {
        cancel.titleLabel.font = _cancelBtnTitleFont;
    }
}
...
-(void)layoutSubviews
{
    ...
    if (@available(iOS 11.0, *) && self.placeHolderCenter) {
        UITextField* textField = [self textField];
        [self setPositionAdjustment:UIOffsetMake((textField.frame.size.width-[self placeHolderTextWidth])/2, 0) forSearchBarIcon:UISearchBarIconSearch];
        textField.frame = CGRectMake(AdaptedWidth(20), 0, kScreenWidth - AdaptedWidth(40), 30);
